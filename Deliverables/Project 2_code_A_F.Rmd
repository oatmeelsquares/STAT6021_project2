...
From the correlation matrix values will range from -1, 0, and 1. Where closeness to -1 indicates a strong negative correlation and closeness to 1 indicates a strong positive correlation. As such, closeness to 0 indicates that there is no correlation. From the predictors mentioned in Question 1 the following appear to have positive correlations:
...
#Condition:
 From the boxplot we can see that waterfront properties appear to have more houses that are a 4 or 5 condition. 
 
 The bar plot shows that a higher proportion of waterfront properties are rated at condition 4 or 5. This may be that waterfront properties have a premium maintenance/environmental maintenance to withstand being near aquatic conditions or how water front properties are marketed to the consumer as being of higher condition status. 
```{r}
##condtion bar graph
library(ggplot2)
...
ggplot(train, aes(x = waterfront, fill = condition, group = condition)) +
  ...
  scale_fill_manual(values = c("red", "blue", "forestgreen", "purple", "yellow")) 
