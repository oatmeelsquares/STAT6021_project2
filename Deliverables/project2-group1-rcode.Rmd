...
```{r}
ggplot(train, aes(x = as.factor(waterfront), fill = condition, group = condition)) +
...
round(cor(train_unfactored[c(7, 2, 3, 4, 9, 10, 15)]), 3)
```
...
train_log$pred <- predict(model1_log, train, type="response")
```
...
The calculated AUC value for the  model is 0.7853352, indicating a strong level of discrimination between positive and negative instances. This AUC value, situated close to 1.0, suggests that the model performs considerably better than random chance/random guessing. In conjunction with the (ROC) curve, the higher the AUC, the more effectively the model distinguishes between true positive and true negative predictions across various probability thresholds.
