...
```{r}
all.models <- leaps::regsubsets(price ~ . , data = train, nbest = 2)
...
Mallow's C~p~ compares each model to the full model to determine which fits the data best. It also penalizes for complexity by subtracting the second term, which will be smaller when p is large.

$C_{p} = \frac{SS_{res}(p)}{MS_{res}(p)} - (n - 2p)$

We are looking for small values with this criterion: the smaller, the better.

```{r}
cp <- sort(summary(all.models)$cp)
cp
```

The best model based on Mallow's C~p~ would be the first one listed here, with coefficients

```{r}
coef(all.models, which.min(cp))
```

However, the five models have relatively low values when compared to the rest of them, so they are worth investigating. 
```{r}
head(coef(all.models, cp), 5)
```
...
The number of bathrooms, and other sqft measurements are highly correlated with sqft_living, so we decided that a visualization for sqft_living would be sufficient to represent all of them. We will keep this in mind when selecting our models because there is likely to be multicollinearity between these predictors, and we will likely only keep one. A similar reasoning combined with low correlationw ith price caused us to leave the number of bedrooms out of our visualizations as well.
